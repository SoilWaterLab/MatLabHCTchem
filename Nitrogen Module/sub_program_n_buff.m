...
% in from 'program_n.m'.  Buffer just removes sediment bound N for now.  It
% does not promote increased denitrification.
...
% This should change with location. Possible resrouces to improve this
% aspect of the model include: The National Atmosphereic Deposition Program
...
% Particulate N accumulated in the buffer (kg/m^2)
bufferN=zeros(numsimdays,numofe);
...
% Particulate N accumulated in the buffer (kg/m^2)
bufferNkgm2=zeros(numsimdays,numofe);
...
for id=numofe+1:1:watrow % start on the second day
    ...
    out=dailyofeNsiM1layTransBuff(id,i,k,numday,applnarea,cropresidue(i,ofe),rootdepth(id),no3surf(ofe),soilNmass(i,ofe),manureN,fertN,applnday,normallai(id),harvestday,plowday,thalfNact,thalfNfrsh,thalfNstab,ps(croptype(id),ofe),oc(croptype(id),ofe),thetaS(croptype(id),ofe),thetaWP(croptype(id),ofe),thetaHO(croptype(id),ofe),thetaLO(croptype(id),ofe),thetathresh,theta(id),kd,fact,fabove,fleft,fharv,fmax,nq,tstep,Q10,Tb,es,kn,cs,kh,ftrans,cnthrsh,kf,fe,fh,minT,no3rain(i,ofe),nh4rain(i,ofe),plantN(i-1,ofe),nh4(i-1,ofe),no3(i-1,ofe),orgNact(i-1,ofe),orgNfrsh(i-1,ofe),orgNstab(i-1,ofe),no3lostovldfUP,no3lostlaftUP,orgNactlostovldUP,orgNfrshlostovldUP,orgNstablostovldUP,cact(i-1,ofe),cfrsh(i-1,ofe),cstab(i-1,ofe),avgT(id),soilTlst(id),ovldf(id),latf(id),perc(id),sedfrac,ofe,buffernum);
    % Mixing depth (m)
    hmix(i,ofe)=mixdepth;
    % Sediment fraction
    ...
    bufferNkgm2(out(3),numofe)=out(21)/applnarea;
...
dailyNpooldata=[nh4,no3,plantN,orgNact,orgNfrsh,orgNstab,cact,cfrsh,cstab,bufferN];
%
% Daily N lost (kg/m^2)
...
dailyNpooldatakgm2=[nh4kgm2,no3kgm2,plantNkgm2,orgNactkgm2,orgNfrshkgm2,orgNstabkgm2,cactkgm2,cfrshkgm2,cstabkgm2,bufferNkgm2];
...
% convert to kg/ha
for k=1:1:mpy
    monthlyavgNerosbymonthhill(k)=mean(monthlyNerosbymonthhill(:,k))*10000*(ofeLength(numofe)/sum(ofeLength)); % must multiply by length last non-buf ofe/sum of all ofe lengths to get in right units for end of the hillslope b/c wepp .wat inputs are cumulative)
...
for k=1:1:numyrs
    yearlyNeroshill(k)=yearlylostNeros(k,numofe).*(ofeLength(numofe)/sum(ofeLength)); % must multiply by length last non-buffer ofe/sum of all ofe lengths to get in right units for end of the hillslope b/c wepp .wat inputs are cumulative)
